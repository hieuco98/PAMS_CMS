...
['$rootScope', '$scope', '$window', '$interval', '$location', '$filter', 'mbookingService', 'NgTableParams','$cookies',
 function ($rootScope, $scope, $window, $interval, $location, $filter, mbookingService, NgTableParams,$cookies) {
	...
	$scope.optionBookingState = [
		{
			id:1,
			name:"Chưa xác nhận/đang chờ"
		},
		{
			id:2,
			name:"Cho phép sử dụng phòng"
		},
		{
			id:3,
			name:"Từ chối"
    ...
    $scope.updateBookingOption=function (campus_area_id,new_option){
		...
		mbookingService.updateBookingEnabledOptionApi(req, function(respData){
	...
	function fnListBookingOrder(campus_area_id){
		...
		mbookingService.listBookingOrderApi(req, function(respData){
			console.log(respData);
			if(respData.code == 200 ) {
	...
	function fnRoomConflictCheck(booking_order){
		...
		mbookingService.RoomConflictCheckApi(reqData, function (respData) {
			console.log(respData);
			if(respData.code == 200 ) {
	...
	function fnListCampusType(){
		var reqData = {};
		mbookingService.ListLoaiCampusAreaApi(reqData, function (respData) {
			console.log(respData);
			if(respData.code == 200 ) {
	...
	function fnListCampusOwnerType(){
		var reqData = {};
		mbookingService.ListOwnerTypeApi(reqData, function (respData) {
			console.log(respData);
			if(respData.code == 200 ) {
	...
	function fnListBookingState(){
		var reqData = {};
		mbookingService.ListBookingStateApi(reqData, function (respData) {
			console.log(respData);
			if(respData.code == 200 ) {
	...
	function fnGetCampusAreaTree(){
		...
		mbookingService.GetCampusAreaTreeApi(reqData, function (respData) {
    		if (respData.code == 200) {
	...
	function fnListTimeScheduleOfRoom(campus_area){
		...
		mbookingService.ListTimeScheduleOfRoomApi(reqData, function (respData) {
			console.log(respData);
			if(respData.code == 200 ) {
  ...
  //---------------------------------------------------------------------------------------------
    $scope.updateBookingOrder=function(order,new_state, reason){
		...
		mbookingService.UpdateBookingOrderApi(reqData, function (respData) {
			if(respData.code == 200){
				fnListBookingOrder();
				alert("Thành công");
	    	}  else{
