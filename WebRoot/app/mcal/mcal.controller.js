...
['$rootScope', '$scope', '$window', '$interval', '$location', '$filter', 'mcalService', 'NgTableParams','$cookies',
    ...
    $scope.tableParamsClasses = new NgTableParams({
        ...
        getData: function($defer,params) {
            ...
            $scope.tableDisplayClasses = orderedData.slice((params.page() - 1) * params.count(), params.page() * params.count());
        }
    });
	...
	$scope.findTimetableClass = function(mSelectedBuild,mSelectedRoom,findDate,mSelectedSemester)
	{
		var reqData = {};
		reqData.session_id = $cookies.get('session_id');
	  ...
	  else
	  {
		...
		mcalService.GetTimetableOfClassApi(reqData, function (respData) {
			...
			if (respData.code == 200) {
				...
				getTimetableByWeek(mSelectedBuild.name,mSelectedRoom.name,week,semester)
				// $scope.floorList = respData.list_area;
				// $scope.mSelectedFloor = $scope.floorList[0];
				// listRoom($scope.mSelectedFloor.id)
				// fnChangeRootProg($scope.modelProgSelected);
...
function renderTimetableOfWeek(start_date,list_classes)
{
	...
	var classes_list =[]
		list_classes.map( function (classes) {
		  ...
		  for(let i=classes.starting_block; i<=classes.ending_block;i++)
		  {
		  var  classes_info = {
	...
	$scope.tt_data1 = [
{
	...
	function loadX(){
		...
		var calendar = new FullCalendar.Calendar(calendarEl, {
			...
			editable: true,
			selectAllow: function(select) {//disable select past dates
